#!/bin/tcsh
  foreach dir (sub-???????)
    ...
    foreach runname (meica*)
        ...
        #next, add high k components to mean tsoc (high k file does not have the mean image in it; it is necessary to compute the mean image of tsoc and add it to high k); need to resample kappa files to tsoc (don’t have bounding box)
